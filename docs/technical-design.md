...
- Shelter navigation is object-driven rather than sign-driven. Generated physical props are layered over the empty classroom background, their texture bounds define mouse interaction, and hover draws a red silhouette outline. The Mission Map is pinned to the left blackboard and opens mission selection; a desk trophy opens Achievements; a hardbound desk book opens Settings. Character Area is omitted from drawing and hit testing until a companion is introduced.
...
- `Combat` owns the running-claw state and its independent cooldown. The slide uses a cubic ease-out to model strong ground friction while integrating to approximately `50` logical X pixels; damage, reset, and map collision resolution can cancel or constrain that displacement safely.
...
- Linxi is constrained by a reusable screen-space safe zone after map collision and camera movement. Its insets use the current rendered body footprint and shadow plus `16` screen pixels, so fixed-room bounds and scrolling-map terminal strips cannot push her artwork beyond the viewport. G mode automatically receives larger insets. Development placement mode draws this safe rectangle in cyan.
