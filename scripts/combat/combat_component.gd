...
func try_attack() -> void:
	...
	host.last_body_attack_id = attack.attack_id
	...
	var target_index: int = host.enemy_component.nearest_target_in_attack(attack.input_tag, attack.range_x, attack.range_depth)
	if target_index < 0:
		return
	var enemy: Dictionary = host.enemies[target_index]
	var was_alive := int(enemy["health"]) > 0
	var raw_damage: int = maxi(host.balance.attack_damage(host.biomass) + attack.damage_bonus, 1)
...
func try_dodge() -> void:
	if host.g_mode:
		...
		return
	if host.digesting or host.player_defeated or host.dodge_time > 0.0 or host.player_height > 0.0: return
