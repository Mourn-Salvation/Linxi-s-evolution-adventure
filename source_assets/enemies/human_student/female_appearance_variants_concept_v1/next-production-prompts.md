...
Status: base design approved by user direction. Do not use the rejected cache image in this folder. Generate from the approved design sheet/reference only.

## Runtime IDs

- `appearance_id = 1`: current black-haired female student
- `appearance_id = 2`: black ponytail female student
- `appearance_id = 3`: blonde long-straight-haired female student
- `appearance_id = 4`: red-haired female student
...
Layout: exact 1 row x 2 cells or 2 rows x 2 cells with only two distinct required poses if the generator needs grid stability. Solid pure `#FF00FF` magenta background. Full body visible, stable feet/bottom baseline, consistent scale, generous padding. No text, no borders, no extra characters, no FX.
